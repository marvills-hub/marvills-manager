...
export class AttachmentService {
  ...
  async uploadFiles(
    ...
    if (!workspaceId) throw new Error('Workspace ID is required to upload attachments.');
    if (!entityId) throw new Error('Project or task ID is required to upload attachments.');
    if (!files.length) return [];
    this.validateFileCount(files.length, existingAttachmentCount);
    const attachments: Attachment[] = [];
    for (const originalFile of files) {
      const attachment = await this.uploadFile(workspaceId, entityType, entityId, originalFile);
      attachments.push(attachment);
    }
    if (attachments.length !== files.length)
      throw new Error('One or more attachments could not be prepared after upload.');
    return attachments;
  }
  ...
  getFileIcon(attachment: Attachment): string {
    ...
    if (type.includes('pdf') || name.endsWith('.pdf')) return 'fa-regular fa-file-pdf';
  ...
  private async uploadFile(
    ...
    const safeFileName = this.createSafeFileName(file.name) || `attachment-${id}`;
    const storagePath = [
      'workspaces',
      workspaceId,
      entityType,
    ...
    if (!url) throw new Error(`Unable to get download URL for "${originalFile.name}".`);
    const attachment: Attachment = {
      id,
      name: prepared.optimized ? prepared.originalName : file.name,
      url,
      storagePath,
      size: file.size,
